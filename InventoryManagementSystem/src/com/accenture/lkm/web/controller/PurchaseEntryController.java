...
public class PurchaseEntryController {
	...
	public ModelAndView generateUnitAndTypeList(@ModelAttribute("purchaseBean") PurchaseBean purchaseBean,
		...
		session.setAttribute("MaterialTypeList", materialTypeBeans);
		//mv.addObject("MaterialTypeList",materialTypeBeans);
		System.out.println("Purchase bean after get"+purchaseBean);
		return mv;
	}
	...
	public List<MaterialCategoryBean> generateCategoryList() throws MicroServiceException {
		System.out.println(materialCategoryConsumer.getMaterialCategoryBeanList());
		List<MaterialCategoryBean> bean=materialCategoryConsumer.getMaterialCategoryBeanList();
	...
	public ModelAndView addPurchaseDetail(@ModelAttribute("purchaseBean") @Valid PurchaseBean purchaseBean,
		...
		String materialCategoryName=(materialCategoryConsumer.hitGetMaterialCategoryById(purchaseBean.getMaterialCategoryId())).getCategoryName();
		mv.addObject("MaterialTypeList",session.getAttribute("MaterialTypeList"));
		mv.addObject("UnitList",session.getAttribute("UnitList"));
		if(result.hasErrors())
		{
			mv.setViewName("PurchaseEntry");      
		}else
		{
			...
			PurchaseBean beans=purchaseService.addPurchaseDetails(purchaseBean);
			mv.setViewName("PurchaseSuccess");
			
			mv.addObject("message","Successfully Purchased with Transaction Id:"+beans.getTransactionId());	
		}
		return mv;
	}
